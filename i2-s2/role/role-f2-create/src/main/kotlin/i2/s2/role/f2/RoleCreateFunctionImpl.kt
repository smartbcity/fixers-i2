...
class RoleCreateFunctionImpl {
	...
	private fun AuthRealmClient.initRole(cmd: RoleCreateCommand) {
		...
		createRole(cmd.realmId, cmd.id, cmd.description, cmd.isClientRole)
		getRoleResource(cmd.realmId, cmd.id).addComposites(compositeRoles)
	}

	private fun AuthRealmClient.createRole(realmId: RealmId, role: RoleId, description: String?, isClientRole: Boolean): RoleRepresentation {
